...
#if defined(CONFIG_MX6_TRUXD01_SOM_EMMC_REWORK) 
...
static iomux_v3_cfg_t const fec1_pads[] = {
	...
	MX6_PAD_JTAG_MOD__GPIO1_IO10 | MUX_PAD_CTRL(NO_PAD_CTRL),  /*Trunexa added*/
};
...
{
	if(is_cpu_type(MXC_CPU_MX6UL))
		puts("Board: i.MX6UL TRUX-iMX6UL-D01-SOM\n");
	else
		puts("Board: i.MX6ULL TRUX-iMX6UL-D01-SOM\n");
	printf("Boot Device: ");
...
{   
	...
	if(is_cpu_type(MXC_CPU_MX6UL))
	{
		setenv("fdt_file","imx6ul-truxd01-som.dtb");
	}
	else 
	{
		setenv("fdt_file","imx6ull-truxd01-som.dtb");
	}
	return 0;
}

void truxd01_fdt_update(void *fdt)
{
        int node;
        int nodeoffset;
        u32 addr_phandle;
	...
	node = fdt_node_check_compatible(fdt, NULL, "trux,ul_truxd01_som");
        if (node < 0)
                return;

        node = fdt_node_check_compatible(fdt, NULL, "trux,ull_truxd01_som");
        if (node < 0)
                return;

        /* TRUXD01: FDT: camera selection */
